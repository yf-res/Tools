...
class PyFastNUC {
public:
    PyFastNUC(py::array_t<float> gain, py::array_t<float> offset) {
        ...
        float* gain_ptr = static_cast<float*>(gain_buf.ptr);
        float* offset_ptr = static_cast<float*>(offset_buf.ptr);

        std::vector<float> gain_vec(gain_ptr, gain_ptr + height * width);
        std::vector<float> offset_vec(offset_ptr, offset_ptr + height * width);
